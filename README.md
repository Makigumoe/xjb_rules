...
但是，你可以用枪械进行近战搏斗。可以想象成，你灵巧的使用枪托和枪管对敌人进行了一连串的砸击。

如果你**单手**持用枪械，那么**持用枪械的手**视为徒手。如果你**双手**持用枪械，那么两只手都视为徒手。
...
**霰弹** 具有该属性的枪械，在射击时会消耗霰弹。一次攻击消耗1颗霰弹，但会喷出6个破片。你选择你射程内的一个生物作为主目标，主目标将会被**6个破片其中的d6个**攻击。而射程内的其他生物，最多可以受到1个破片攻击。**攻击生物的破片，全部加起来不能超过6个，但可以少于6个**。每个破片单独进行攻击检定，以及伤害上单独获得来自敏捷和其他的加值。同时，主目标需要进行一次dc10的力量（运动）检定，如果检定失败，则陷入**倒地**状态。
...
**抵近射击** 允许在敌对生物5尺范围内进行远程攻击，同时不会因为在敌对生物5尺范围内进行远程攻击而具有劣势。
...
霰弹枪|50gp|d4穿刺|10磅|霰弹（射程20/60），双手，冲锋，抵近射击
...
* 你使用手枪时，手枪具有**抵近射击**属性。
* 在**使用近战武器**进行近战攻击之后，对同一目标**使用手枪**进行射击，不会因为身边5尺范围内存在敌对生物而具有劣势；取而代之，这次射击有优势。
* 在**使用手枪**进行远程攻击之后，对同一目标进行近战攻击具有优势。
